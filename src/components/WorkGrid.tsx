...
const PortfolioGrid = () => {
  ...
  return (
    <section className={`w-full px-[16px] sm:px-[32px] md:px-[48px] lg:px-[60px] py-6 transition-all duration-1000 ease-out ${
        ...
        {gridLayout.map((row, rowIndex) => (
          ...
          >
            {row.map((projectIndex) => (
              <div key={projectIndex} className={row.length > 1 ? 'flex-1' : 'w-full'}>
                <WorkCard {...projects[projectIndex]} />
              </div>
            ))}
          </div>
        ))}
